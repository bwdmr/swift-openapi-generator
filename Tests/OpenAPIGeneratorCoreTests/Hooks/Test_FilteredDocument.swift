...
struct Test_FilteredDocument {
    ...
    func verifyDocumentMatches(
        ...
        do { filteredDocument = try filter.filter(document) }
        catch {
            #expect(Bool(false), "Filter threw error: \(error)")
            return
        }
    ...
    func testDocumentFilter() throws {
        ...
        verifyDocumentMatches(filtering: document, filter: DocumentFilter(), hasPaths: [], hasOperations: [], hasSchemas: [])
        verifyDocumentMatches(
            filtering: document,
