...
struct ValidateDocTests {
    ...
    func testSchemaWarningIsNotFatal() throws {
        ...
        #expect(diagnostics.count == 1, Testing.Comment(stringLiteral: "Expected exactly 1 diagnostic for schema warning"))
    }
    
    
    @Test("Valid content types pass validation")
    func testValidateContentTypes_validContentTypesInResponses() {
        let doc = OpenAPI.Document(
            info: .init(title: "Test", version: "1.0.0"),
            servers: [],
            paths: [
    ...
    func testValidateContentTypes_invalidContentTypesInRequestBody() {
        let doc = OpenAPI.Document(
            info: .init(title: "Test", version: "1.0.0"),
            servers: [],
            paths: [
    ...
    // MARK: Duplicate Function
    @Test("Invalid content types in components request bodies fail validation")
    func testValidateContentTypes_invalidContentTypesInComponentsRequests() {
        let doc = OpenAPI.Document(
            info: .init(title: "Test", version: "1.0.0"),
            servers: [],
            paths: [
    ...
    func testValidateContentTypes_invalidContentTypesInResponses() {
        let doc = OpenAPI.Document(
            info: .init(title: "Test", version: "1.0.0"),
            servers: [],
            paths: [
    ...
    func testValidateReferences_referenceNotFoundInComponents() throws {
        ...
        do {
            ...
            #expect(Bool(false), "Expected validateReferences to throw an error")
        } catch let error {
            #expect(error is Diagnostic)
    ...
    func testValidateReferences_foundExternalReference() throws {
        ...
        do {
            ...
            #expect(error.localizedDescription == "error: External references are not supported. [context: location=/path/GET/responses/200, reference=ExternalURL]")
        } catch {
            #expect(Bool(false), "Expected Diagnostic error, but got \(type(of: error))")
        }
    }
    ...
    func testValidateTypeOverrides() throws {
        ...
        #expect(diagnostics.count == 1, Testing.Comment(stringLiteral: "Expected exactly one diagnostic"))
